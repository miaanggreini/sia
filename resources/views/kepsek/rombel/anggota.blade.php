...
@php
    ...
    $judulAnggota = $isRombelTahunAktif
        ? 'Anggota Rombel Aktif'
        : 'Riwayat Anggota Rombel';

    $deskripsiAnggota = $isRombelTahunAktif
...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Tingkat:
                <span class="font-semibold text-gray-700">{{ $rombel->tingkat ?? '—' }}</span>
                <span class="mx-1">|</span>
                Wali:
                <span class="font-semibold text-gray-700">{{ $waliKelasNama }}</span>
                <span class="mx-1">|</span>
                TA:
                <span class="font-semibold text-gray-700">{{ $tahunAjaranText }}</span>
            </p>
            ...
            <div class="mt-3 flex flex-wrap items-center gap-2 text-sm">
                ...
                @if($isRombelTahunAktif)
                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold
                        ...
                        Sisa: {{ $kapasitas > 0 ? $sisaKapasitas . ' siswa' : '-' }}
                    </span>
                @else
        ...
        <div class="flex flex-row items-center gap-3">
            @if($isRombelTahunAktif)
                @if($isKelasXII)
                    <a href="{{ route('kepala_sekolah.data.rombel.kelulusan.form', $rombel) }}"
                       class="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 transition">
                        Kelulusan Kelas XII
                    </a>
                @else
                    <a href="{{ route('kepala_sekolah.data.rombel.kenaikan.form', $rombel) }}"
                       class="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 transition">
                        Kenaikan Kelas
                    </a>
                @endif
            @endif

            <a href="{{ route('kepala_sekolah.data.rombel.index', $backParams) }}"
               class="inline-flex items-center rounded-xl border bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
                Kembali
            </a>
        </div>
    </div>
    ...
    {{-- MODE HISTORI --}}
    @unless($isRombelTahunAktif)
        <div class="rounded-2xl border border-amber-300 bg-amber-50 px-5 py-4 text-sm text-amber-800">
    ...
    <div class="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div class="flex flex-col gap-2 border-b px-5 py-4 md:flex-row md:items-center md:justify-between">
            ...
            <span class="inline-flex w-fit rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">
                {{ $labelTotal }}: {{ $jumlahAnggota }} siswa
            </span>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($anggotaView as $i => $row)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-4">
                                ...
                                @if(!$isRombelTahunAktif && isset($row->status_keanggotaan))
                                    <div class="mt-1 text-xs text-gray-400">
                                        Status riwayat:
                                        {{ (int) $row->status_keanggotaan === 1 ? 'Aktif saat tahun ajaran tersebut' : 'Pernah terdaftar' }}
                                    </div>
                                @endif
                            </td>
                            ...
                            @if($isRombelTahunAktif)
                                <td class="px-5 py-4 text-right">
                                    <form method="POST"
                                          action="{{ route('kepala_sekolah.data.rombel.anggota.destroy', [$rombel, $s->id]) }}"
                                        ...
                                        <button type="submit"
                                                class="rounded-xl border border-red-200 bg-white px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50 transition">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $isRombelTahunAktif ? 5 : 4 }}" class="px-5 py-10 text-center text-sm text-gray-500">
                                {{ $isRombelTahunAktif
    ...
    {{-- CARD TAMBAH ANGGOTA: HANYA UNTUK TA AKTIF --}}
    @if($isRombelTahunAktif)
        <div class="overflow-hidden rounded-2xl border bg-white shadow-sm">
            ...
            <div class="p-5">
                <form method="GET"
                      action="{{ route('kepala_sekolah.data.rombel.anggota', array_merge(['rombel' => $rombel->id], $anggotaFilterParams)) }}"
                    ...
                    @if(request('q'))
                        <a href="{{ route('kepala_sekolah.data.rombel.anggota', array_merge(['rombel' => $rombel->id], $anggotaFilterParams)) }}"
                ...
                <div class="mb-3 flex flex-col gap-1 text-xs text-gray-500 md:flex-row md:items-center md:justify-between">
                    ...
                    <span>
                        Total anggota aktif:
                        {{ $jumlahAnggota }}{{ $kapasitas > 0 ? ' / ' . $kapasitas : '' }}
                    </span>
                </div>

                <form method="POST"
                      action="{{ route('kepala_sekolah.data.rombel.anggota.store', $rombel) }}">
                    @csrf

                    <input type="hidden"
                           name="tahun_ajaran_id"
                           value="{{ $rombel->tahun_ajaran_id ?? ($tahunAktif->id ?? '') }}">
