{{-- resources/views/kepsek/rombel/kenaikan.blade.php --}}
...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3 text-sm text-indigo-800">
        <div class="font-medium mb-1">Aturan evaluasi kenaikan kelas</div>
        <ul class="list-disc pl-5 space-y-1 text-xs">
            <li>Nilai harus lengkap untuk seluruh mapel pada rombel ini.</li>
            <li>Kehadiran minimal <strong>90%</strong>.</li>
            <li>Sikap minimal <strong>Baik</strong>.</li>
            <li>Siswa yang tidak dicentang akan tetap berada di rombel asal.</li>
        </ul>
    </div>

    {{-- FORM --}}
    <form method="POST" action="{{ route('kepala_sekolah.data.rombel.kenaikan.proses', $rombel) }}" class="space-y-6">
        @csrf

        {{-- PILIH ROMBEL TUJUAN --}}
        <div class="bg-white rounded-lg shadow border">
            <div class="px-4 py-3 border-b">
                <h2 class="text-sm font-semibold text-gray-800">Pilih Rombel Tujuan</h2>
                <p class="text-xs text-gray-500">
                    Siswa yang memenuhi syarat dan dicentang akan dipindah ke rombel tujuan.
                </p>
            </div>
                ...
                <select name="rombel_tujuan_id"
                    ...
                    @foreach ($rombelsTujuan as $r)
                        <option value="{{ $r->id }}" {{ old('rombel_tujuan_id') == $r->id ? 'selected' : '' }}>
                            {{ $r->nama_rombel }}
                            (Tingkat {{ $r->tingkat ?? '-' }}
                            @if($r->tahunAjaran) • {{ $r->tahunAjaran->nama }} @endif)
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="bg-white rounded-lg shadow border overflow-hidden">
            <div class="px-4 py-3 border-b flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                    <h2 class="text-sm font-semibold text-gray-800">Evaluasi Siswa untuk Kenaikan Kelas</h2>
                    <p class="text-xs text-gray-500">
                            ...
                            <tr class="hover:bg-gray-50"
                                ...
                                <td class="px-4 py-3 align-top">
                                    @if($ev && $ev->nilai_lengkap)
                                        ...
                                        <div class="text-xs text-gray-500 mt-1">
                                            {{ $ev->jumlah_nilai }}/{{ $ev->jumlah_mapel }} mapel
                                        </div>
                                    @else
                                ...
                                <td class="px-4 py-3 align-top">
                                    <select name="sikap[{{ $s->id }}]"
                                            class="sikap-select w-full min-w-[130px] border-gray-300 rounded-md text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-6 text-center text-gray-500">
                                    Belum ada anggota di rombel ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="flex justify-end">
            <button type="submit"
                    class="px-5 py-2.5 rounded bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">
                Proses Kenaikan
            </button>
        </div>
    </form>
