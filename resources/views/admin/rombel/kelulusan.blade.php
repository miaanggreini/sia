{{-- resources/views/admin/rombel/kelulusan.blade.php --}}
...
<div class="max-w-7xl mx-auto space-y-6">
  ...
  <div class="bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3 text-sm text-indigo-800">
    ...
    <ul class="list-disc pl-5 space-y-1 text-xs">
      ...
      <li>Siswa yang dicentang dan memenuhi syarat akan diubah statusnya menjadi <strong>lulus</strong>.</li>
    </ul>
  </div>

  {{-- FILTER CARI --}}
  <div class="bg-white rounded-lg shadow border p-4">
    <form method="GET" action="{{ route('admin.rombel.kelulusan.form', $rombel) }}" class="flex flex-col md:flex-row gap-3">
      <input type="text"
             name="q"
             value="{{ $q ?? request('q') }}"
  ...
  {{-- FORM --}}
  <form method="POST" action="{{ route('admin.rombel.kelulusan.proses', $rombel) }}" class="space-y-6">
    @csrf
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody class="divide-y">
            @forelse($siswa as $row)
              @php
                $ev = $evaluasiMap->get($row->id);
                $oldSikap = old("sikap.{$row->id}", '');
              @endphp
              ...
              <tr class="hover:bg-gray-50"
                ...
                <td class="px-4 py-3 align-top">
                  <input type="checkbox"
                         ...
                         {{ in_array($row->id, old('siswa_ids', [])) ? 'checked' : '' }}>
                </td>

                <td class="px-4 py-3 align-top">
                  <div class="font-medium text-gray-900">{{ $row->nis ?? '-' }}</div>
                  <div class="text-xs text-gray-500">NISN: {{ $row->nisn ?? '-' }}</div>
                </td>
