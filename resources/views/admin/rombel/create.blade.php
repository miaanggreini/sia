...
<div class="px-4 max-w-none">
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-semibold text-gray-800">Tambah Rombel</h1>
      <p class="text-sm text-gray-500 mt-1">
        Buat rombel baru berdasarkan tahun ajaran, wali kelas, ruang kelas, dan mata pelajaran.
      </p>
    </div>
  ...
  <form id="form-rombel" method="POST" action="{{ route('admin.rombel.store') }}" class="space-y-6">
    @csrf

    {{-- INFORMASI ROMBEL --}}
    <div class="bg-white rounded-2xl shadow border overflow-hidden">
      <div class="px-5 py-4 border-b">
        <h2 class="font-semibold text-gray-800">Informasi Rombel</h2>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-5">
        ...
        <div>
          ...
          <select name="tahun_ajaran_id"
            ...
            @foreach(($daftarTahunAjaran ?? collect()) as $ta)
              <option value="{{ $ta->id }}" @selected(old('tahun_ajaran_id', $tahunAktif->id ?? '') == $ta->id)>
                {{ $ta->nama_tahun ?? $ta->label ?? '-' }}
                @if(($ta->status ?? null) === 'aktif') — Aktif @else — Nonaktif @endif
              </option>
            @endforeach
          </select>
        </div>
        ...
        <div>
          ...
          <input type="text"
                 ...
                 placeholder="Contoh: XIA, XIB, XIIA"
                 required>
        </div>
        ...
        <div>
          ...
          <select name="ruang_kelas_id"
                  class="w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-indigo-500">
            <option value="">— Pilih Ruang Kelas —</option>
            @foreach(($ruangKelas ?? collect()) as $rk)
              <option value="{{ $rk->id }}" @selected(old('ruang_kelas_id') == $rk->id)>
                {{ $rk->nama ?? $rk->nama_ruang ?? '-' }}
                @if(!empty($rk->kapasitas))
    ...
    {{-- MAPEL --}}
    @php
      $selected = collect(old('mapel_ids', []))->map(fn($v) => (int) $v)->values()->all();
    @endphp

    <div class="bg-white rounded-2xl shadow border overflow-hidden">
      <div class="px-5 py-4 border-b">
        <h2 class="font-semibold text-gray-800">Mata Pelajaran</h2>
        <p class="text-sm text-gray-500">
          Centang mata pelajaran yang diajarkan pada rombel ini.
        </p>
      </div>
      ...
      <div id="mp-list" class="w-full max-h-72 overflow-auto divide-y">
        @forelse(($mapel ?? collect()) as $m)
          ...
          <label class="group flex items-center gap-3 px-5 py-2.5 hover:bg-indigo-50/50 cursor-pointer"
                 data-name="{{ \Illuminate\Support\Str::lower($nama) }}"
                 data-kel="{{ \Illuminate\Support\Str::lower($kel) }}">
            <input type="checkbox"
                   class="mp-check peer rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                   name="mapel_ids[]"
                   value="{{ $id }}"
      ...
      <div class="px-5 py-3 border-t bg-gray-50/60">
        <div class="flex items-center gap-2 text-xs text-gray-600">
          <span id="mp-count" class="inline-flex items-center px-2 py-0.5 rounded bg-indigo-50 text-indigo-700">
            0 dipilih
          </span>
...
(function(){
  ...
  btnAll?.addEventListener('click', () => {
    [...list.querySelectorAll('label[data-name]')].forEach(el => {
      ...
      const c = el.querySelector('.mp-check');
      if (c) c.checked = true;
    });
    refreshChips();
  });
